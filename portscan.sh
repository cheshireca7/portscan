...
: "${d:=1}" "${t:=1}" "${w:=1}" "${o:=false}" "${f:=$(date "+%d%m%Y_%H%M%S")_portscan_results.txt}"
...
echo -ne "[\033[34mI\033[0m] Scan started"; [[ -z ${a} ]] && echo -n ". Scanning commom ports..." && a="${dp}"; if [[ ${a} =~ ${ar} ]];then echo && p "${a}"; else u; fi
echo -ne "\n[\033[34mI\033[0m] Scan finished"; if [[ -s ${f} ]]; then sort -V "${f}" -o "${f}" && echo -e ". Results stored in ${f}"; else rm -f "${f}" &>/dev/null && ${o} && echo ". No open ports"; fi;
